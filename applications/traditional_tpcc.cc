...
//----------------------------------------------------
void Traditional_TPCC::NewTxn(Txn* txn, uint64_t txn_id) const {
  int percent = rand() % 100;

  if (percent < 50) {
    NewOrderTxn(txn, txn_id);
  } else {
    PaymentTxn(txn, txn_id);
  }
}

//--------------------------------------------------------

void Traditional_TPCC::NewOrderTxn(Txn* txn, uint64_t txn_id) const {
...
//-----------------------------------------------------------------
void Traditional_TPCC::PaymentTxn(Txn* txn, uint64_t txn_id) const {
  // Set the transaction's standard attributes
  txn->SetTxnId(txn_id);
...
//------------------------------------------------------------------
int Traditional_TPCC::Execute(Txn* txn) const {
...
int Traditional_TPCC::ExecuteNewOrderTxn(Txn* txn) const {
  ...
  table_key = txn->GetReadWriteSet(0);

 
  assert(table_key.table_id == 1);
  District* district = (District*)storage_->ReadRecord(table_key.table_id, table_key.key);
  float d_tax = district->d_tax;
  uint32_t order_id = district->d_next_o_id;
  district->d_next_o_id += 1;
  uint32_t d_id = TPCCKeyGen::get_district_key(table_key.key);
...
//std::cout<<"customer, w:"<<x_w<<". d:"<<x_d<<". x_c:"<<x_c<<".\n"<<std::flush;
  ...
  for (uint32_t i = 0; i < num_items; i++) {
    table_key = txn->GetReadWriteSet(i + 1);
    assert(table_key.table_id == 8);

    uint32_t item_id = TPCCKeyGen::get_stock_key(table_key.key);
    Item* item = (Item*)storage_->ReadRecord(7, item_id);
    Stock* stock = (Stock*)storage_->ReadRecord(table_key.table_id, table_key.key);
    ...
    uint32_t s_w_id = TPCCKeyGen::get_warehouse_key(table_key.key);
    if (s_w_id != w_id) {
      is_local = false;
...
//------------------------------------------------------------------
int Traditional_TPCC::ExecutePaymentTxn(Txn* txn) const {

 // float h_amount = txn->tpcc_args.payment_amount;
    float h_amount = 100;

  TableKey table_key = txn->GetReadWriteSet(0);
//TableKey table_key = txn->GetReadSet(0);
  assert(table_key.table_id == 0);
  Warehouse* warehouse = (Warehouse*)storage_->ReadRecord(table_key.table_id, table_key.key);
  uint32_t w_id = TPCCKeyGen::get_warehouse_key(table_key.key);
  ...
  table_key = txn->GetReadWriteSet(1);
//table_key = txn->GetReadWriteSet(0);
  assert(table_key.table_id == 1);
  District* district = (District*)storage_->ReadRecord(table_key.table_id, table_key.key);
  uint32_t d_id = TPCCKeyGen::get_district_key(table_key.key);
  ...
  table_key = txn->GetReadWriteSet(2);
//table_key = txn->GetReadWriteSet(1);
  assert(table_key.table_id == 2);
  Customer* customer = (Customer*)storage_->ReadRecord(table_key.table_id, table_key.key);
  uint32_t c_id = TPCCKeyGen::get_customer_key(table_key.key);
  uint32_t c_w_id = TPCCKeyGen::get_warehouse_key(table_key.key);
  uint32_t c_d_id = TPCCKeyGen::get_district_key(table_key.key);
  ...
  storage_->InsertRecord(txn->GetWorkerId(), 3, table_key.key, (void*)&hist);
...
uint32_t Traditional_TPCC::LookupPartition(const uint64_t& key) const {
  return 0;
}

uint32_t Traditional_TPCC::GetTableNum() const {
  return 9;
}

//--------------------------------------------------------------------
void Traditional_TPCC::InitializeStorage() const {
...
int Traditional_TPCC::Execute2(LockUnit* lock_unit) const {
  return 0;
}
int Traditional_TPCC::Rollback(LockUnit* lock_unit) const {
  // Do something to simulate the rollback
      int x = 1;
      for(int i = 0; i < 100; i++) {
        x = x*x+1;
        x = x+10;
...
void Traditional_TPCC::InitializeTable(uint32_t table_id) const {  
}
